...
gulp.task('build:backend', function () {
    src.msbuild = '../ReactAspNetWebApi.BackEnd/**/*.{cs,config}';

    return gulp.src("../ReactAspNetWebApi.sln")
     .pipe(msbuild({
         targets: ['Clean', 'Build'],
         toolsVersion: 4.0 // Default: 4.0,
         //logCommand: true,
         //stdout: true
     })
     );
...
// Run IIS Express
gulp.task('run:backend', function () {
    var configPath = path.join(__dirname, '../.vs/config/applicationHost.config');

    iisexpress({
        siteNames: ['ReactAspNetWebApi.BackEnd'],
        configFile: configPath
    });
});

// open browser with backend
gulp.task('open:backend', function () {
    var urlToOpen = 'http://localhost:8450/api/values';

    gulp.src('').pipe(open({ uri: urlToOpen }));
})

// Serve Backend
gulp.task('serve:backend', ['clean'], function (cb) {
    runSequence('build:backend', 'run:backend', 'open:backend');
});